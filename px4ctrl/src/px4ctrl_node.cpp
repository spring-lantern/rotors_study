...
int main(int argc, char *argv[])
{
    ...
    if(!param.use_sim)
    {
        ...
        odom_sub = nh.subscribe<nav_msgs::Odometry>("odom_sim",
                                                    100,
                                                    boost::bind(&Odom_Data_t::feed, &fsm.odom_data, _1),
                                                    ros::VoidConstPtr(),
                                                    ros::TransportHints().tcpNoDelay());
    ...
    if(!param.use_sim)
    {
        ros::Subscriber imu_sub =
            nh.subscribe<sensor_msgs::Imu>("/mavros/imu/data", // Note: do NOT change it to /mavros/imu/data_raw !!!
                                        ...
                                        ros::VoidConstPtr(),
                                        ros::TransportHints().tcpNoDelay());
        // imu_sub = nh.subscribe<sensor_msgs::Imu>("imu", // Note: do NOT change it to /mavros/imu/data_raw !!!
        //                                         100,
        //                                         boost::bind(&Imu_Data_t::feed, &fsm.imu_data, _1),
        //                                         ros::VoidConstPtr(),
        //                                         ros::TransportHints().tcpNoDelay());
    }else{
        imu_sub = nh.subscribe<sensor_msgs::Imu>("imu_sim", // Note: do NOT change it to /mavros/imu/data_raw !!!
                                                100,
                                                boost::bind(&Imu_Data_t::feed, &fsm.imu_data, _1),
                                                ros::VoidConstPtr(),
                                                ros::TransportHints().tcpNoDelay());
    ...
    fsm.control_sim_pub = nh.advertise<mav_msgs::Actuators>("cmd_sim", 10); // ljx
