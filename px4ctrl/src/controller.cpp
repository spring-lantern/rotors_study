...
Controller::Controller(Parameter_t &param_) : param(param_)
{
  ...
  // ljx
  KBr(0) = param.gain.KBrR;
  KBr(1) = param.gain.KBrP;
  KBr(2) = param.gain.KBrY;
...
// ljx
Eigen::Vector3d Controller::computeFeedBackControlTorques(
    const Eigen::Vector3d &des_bodyrates,
    const Eigen::Vector3d &est_bodyrates,
    const Parameter_t &param)
{
  // Compute the error bodyrates
  const Eigen::Vector3d bodyrates_error = des_bodyrates - est_bodyrates;

  // Compute desired torques from control error
  Eigen::Vector3d torques;

  torques.x() = KBr(0) * bodyrates_error.x();
  torques.y() = KBr(1) * bodyrates_error.y();
  torques.z() = KBr(2) * bodyrates_error.z();

  if (torques.x() > kMaxTorquesFeedback_)
    torques.x() = kMaxTorquesFeedback_;
  if (torques.x() < -kMaxTorquesFeedback_)
    torques.x() = -kMaxTorquesFeedback_;
  if (torques.y() > kMaxTorquesFeedback_)
    torques.y() = kMaxTorquesFeedback_;
  if (torques.y() < -kMaxTorquesFeedback_)
    torques.y() = -kMaxTorquesFeedback_;
  if (torques.z() > kMaxTorquesFeedback_)
    torques.z() = kMaxTorquesFeedback_;
  if (torques.z() < -kMaxTorquesFeedback_)
    torques.z() = -kMaxTorquesFeedback_;
...
// ljx
Eigen::Vector4d Controller::computeMotorsSpeed(
    const double &normalized_total_thrust,
    const Eigen::Vector3d &feedback_torques)
{
  Eigen::Vector4d motors_speed;
  Eigen::Vector4d motors_thrust;

  double max_thrust = thr2acc * param.mass;
  double pi_4 = 0.7853981633974483;
  double lsp = param.arm_length * sin(pi_4);
  double lcp = param.arm_length * cos(pi_4);
  
  double total_thrust = normalized_total_thrust * max_thrust;
  Eigen::Vector3d torques = feedback_torques * param.arm_length * sin(pi_4) * max_thrust;

  Eigen::Vector4d thrust_plus_torques;
  thrust_plus_torques[0] = total_thrust;
  thrust_plus_torques[1] = torques[0];
  thrust_plus_torques[2] = torques[1];
  thrust_plus_torques[3] = torques[2];

  Eigen::Matrix<double, 4, 4> mixer_matrix;
  Eigen::Matrix<double, 4, 4> inv_mixer_matrix;

  mixer_matrix << 1, 1, 1, 1, 
                -lcp, lcp, lcp, -lcp,
                -lsp, lsp, -lsp, lsp,
                -param.zmoment_thrust_coeff, -param.zmoment_thrust_coeff, param.zmoment_thrust_coeff, param.zmoment_thrust_coeff;

  inv_mixer_matrix = mixer_matrix.transpose()*(mixer_matrix*mixer_matrix.transpose()).inverse();

  motors_thrust = inv_mixer_matrix * thrust_plus_torques;
  for (size_t i=0; i<4; i++)
  {
    motors_speed[i] = sqrt(std::min(std::max(motors_thrust[i], param.min_thrust), param.max_thrust) / param.kf);
  }
