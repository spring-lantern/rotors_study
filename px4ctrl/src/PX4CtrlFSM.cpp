...
void PX4CtrlFSM::process()
{
	...
	else
	{
		if (state == AUTO_HOVER || state == CMD_CTRL)
		{
			publish_command_sim(u, now_time);
		}
...
// ljx
void PX4CtrlFSM::publish_command_sim(const Controller_Output_t &u, const ros::Time &stamp)
{
	Eigen::Vector3d feedback_torques = controller.computeFeedBackControlTorques(u.bodyrates, imu_data.w, param);
	Eigen::Vector4d motors_speed = controller.computeMotorsSpeed(u.thrust, feedback_torques);

	mav_msgs::Actuators command;
	command.header.stamp = stamp;
	command.angular_velocities.clear();
	for(int i = 0; i < motors_speed.size(); i++){
		command.angular_velocities.push_back(motors_speed[i] * M_PI / 30.0);
	}
	control_sim_pub.publish(command);	
}
