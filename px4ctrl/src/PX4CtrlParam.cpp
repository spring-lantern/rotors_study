...
void Parameter_t::config_from_ros_handle(const ros::NodeHandle &nh)
{
	...
	read_essential_param(nh, "gain/KBrR", gain.KBrR);
	read_essential_param(nh, "gain/KBrP", gain.KBrP);
	read_essential_param(nh, "gain/KBrY", gain.KBrY);
	...
	read_essential_param(nh, "zmoment_thrust_coeff", zmoment_thrust_coeff);
	read_essential_param(nh, "kf", kf);
	...
	read_essential_param(nh, "sim/use_sim", use_sim); // ljx
