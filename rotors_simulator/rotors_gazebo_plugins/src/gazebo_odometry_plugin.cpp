...
void GazeboOdometryPlugin::OnUpdate(const common::UpdateInfo& _info) {
  ...
  if (gazebo_sequence_ % measurement_divisor_ == 0) {
    ...
    //ZRB warning: 2022.12.15 modify velocity to world frame
    Eigen::Vector3d linear_velocity_bodyframe(gazebo_linear_velocity.X(), gazebo_linear_velocity.Y(), gazebo_linear_velocity.Z());
    Eigen::Quaterniond odom_quaternion(gazebo_pose.Rot().W(), gazebo_pose.Rot().X(), gazebo_pose.Rot().Y(), gazebo_pose.Rot().Z() );
    Eigen::Matrix3d odom_R = odom_quaternion.toRotationMatrix();	
    Eigen::Vector3d linear_velocity_worldframe = odom_R * linear_velocity_bodyframe;

    odometry.mutable_twist()->mutable_twist()->mutable_linear()->set_x(
        linear_velocity_worldframe[0]);
    odometry.mutable_twist()->mutable_twist()->mutable_linear()->set_y(
        linear_velocity_worldframe[1]);
    odometry.mutable_twist()->mutable_twist()->mutable_linear()->set_z(
        linear_velocity_worldframe[2]);
